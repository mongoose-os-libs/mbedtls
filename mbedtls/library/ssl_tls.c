...
static int ssl_encrypt_buf( mbedtls_ssl_context *ssl )
{
    ...
    {
        unsigned char *p = ssl->dout_msg + ssl->out_msglen;
#if defined(MBEDTLS_SSL_PROTO_SSL3)
        if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 )
        {
            ...
            memcpy( ssl->dout_msg + ssl->out_msglen, mac, ssl->transform_out->maclen );
        }
        else
#endif
#if defined(MBEDTLS_SSL_PROTO_TLS1) || defined(MBEDTLS_SSL_PROTO_TLS1_1) || \
        ...
        mode == MBEDTLS_MODE_CCM )
    {
        ...
        MBEDTLS_SSL_DEBUG_BUF( 4, "IV used", ssl->dout_iv,
    ...
    if( mode == MBEDTLS_MODE_CBC )
    {
        ...
        out_space = ssl->out_msglen + ssl->transform_out->ivlen;
...
#error Compression does not work with dynamic buffers!
    // If compression is desired, we must find out compressed size upfront.
...
#error Compression does not work with dynamic buffers!
    // If compression is desired, we must find out decompressed size upfront.
...
int mbedtls_ssl_resize_out_buf( mbedtls_ssl_context *ssl,
    ...
    size_t want_size = (*p - ssl->dout_buf) + size;
    ...
    // TODO(rojer): mbedtls_realloc.
    new_buf = realloc(old_buf, want_size);
...
int mbedtls_ssl_flush_output( mbedtls_ssl_context *ssl )
{
    ...
    while( ssl->out_left > 0 )
    {
        ...
        buf = ssl->dout_hdr + mbedtls_ssl_hdr_len( ssl ) +
...
static void ssl_swap_epochs( mbedtls_ssl_context *ssl )
{
    ...
    memcpy( tmp_out_ctr,                 ssl->dout_ctr,                8 );
    memcpy( ssl->dout_ctr,                ssl->handshake->alt_out_ctr, 8 );
    ...
    {
        ssl->dout_msg = ssl->dout_iv + ssl->transform_out->ivlen -
                                     ssl->transform_out->fixed_ivlen;
    }
    else
...
int mbedtls_ssl_resend( mbedtls_ssl_context *ssl )
{
    ...
    while( ssl->handshake->cur_msg != NULL )
    {
        ...
        MBEDTLS_SSL_DEBUG_BUF( 3, "resent handshake message header", ssl->dout_msg, 12 );
 ...
 * Uses ssl->out_msgtype, ssl->out_msglen and bytes at ssl->dout_msg.
 */
int mbedtls_ssl_write_record( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE )
    {
        ...
        ssl->dout_msg[1] = (unsigned char)( ( len - 4 ) >> 16 );
        ssl->dout_msg[2] = (unsigned char)( ( len - 4 ) >>  8 );
        ssl->dout_msg[3] = (unsigned char)( ( len - 4 )       );
        ...
        if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
        {
            ...
            if( ( ret = mbedtls_ssl_grow_out_buf( ssl, &p, len + 8, NULL ) ) != 0 )
            {
                MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_ssl_grow_out_buf", ret );
                return( ret );
            }
            memmove( ssl->dout_msg + 12, ssl->dout_msg + 4, len - 4 );
            ssl->out_msglen += 8;
            len += 8;
        ...
        if( out_msg_type != MBEDTLS_SSL_HS_HELLO_REQUEST )
            ssl->handshake->update_checksum( ssl, ssl->dout_msg, len );
    }
    ...
    if( !done )
    {
        ...
        MBEDTLS_SSL_DEBUG_MSG( 3, ( "output record: msgtype = %d, "
                       ...
                       ssl->dout_hdr[0], ssl->dout_hdr[1], ssl->dout_hdr[2],
                     ( ssl->dout_len[0] << 8 ) | ssl->dout_len[1] ) );

        MBEDTLS_SSL_DEBUG_BUF( 4, "output record sent to network",
                       ssl->dout_hdr, mbedtls_ssl_hdr_len( ssl ) + ssl->out_msglen );
    }
...
static int ssl_reassemble_dtls_handshake( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->in_left > ssl->next_record_offset )
    {
        ...
        if( ssl->in_left > MBEDTLS_SSL_MAX_BUFFER_LEN -
...
int mbedtls_ssl_write_certificate( mbedtls_ssl_context *ssl )
{
    ...
    while( crt != NULL )
    {
        unsigned char *p = ssl->dout_msg + i;
        n = crt->raw.len;
        if( ( ret = mbedtls_ssl_grow_out_buf( ssl, &p, i + n + 3, NULL ) ) != 0 )
        {
            MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_ssl_grow_out_buf", ret );
            return( MBEDTLS_ERR_SSL_CERTIFICATE_TOO_LARGE );
        }
...
int mbedtls_ssl_write_finished( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_2 )
    {
        ssl->dout_msg = ssl->dout_iv + ssl->transform_negotiate->ivlen -
                        ssl->transform_negotiate->fixed_ivlen;
    }
    else
        ssl->dout_msg = ssl->dout_iv;
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        memcpy( ssl->handshake->alt_out_ctr, ssl->dout_ctr, 8 );

        /* Set sequence_number to zero */
        memset( ssl->dout_ctr + 2, 0, 6 );
                       ...
                       const mbedtls_ssl_config *conf )
{
    ...
    {
        /* At this point in_buf is NULL and it's ok, we only want offsets. */
        ssl->dout_hdr = ssl->dout_buf;
        ssl->dout_ctr = ssl->dout_buf +  3;
        ssl->dout_len = ssl->dout_buf + 11;
        ssl->dout_iv  = ssl->dout_buf + 13;
        ssl->dout_msg = ssl->dout_buf + 13;
...
static int ssl_check_ctr_renegotiate( mbedtls_ssl_context *ssl )
{
    ...
    out_ctr_cmp = memcmp( ssl->dout_ctr + ep_len,
                          ssl->conf->renego_period + ep_len, 8 - ep_len );
...
void mbedtls_ssl_free( mbedtls_ssl_context *ssl )
{
    ...
    {
        mbedtls_zeroize( ssl->dout_buf, ssl->dout_buf_size );
        mbedtls_free( ssl->dout_buf );
    }

    if( ssl->in_buf != NULL )
    {
        mbedtls_zeroize( ssl->in_buf, ssl->in_buf_size );
